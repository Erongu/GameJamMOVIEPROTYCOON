...
function Load(){
	...
	$('.interface').mousedown(function () {
		if ($(this).css('z-index') != z_index)
			...
			$(this).css({'top': ($(window).height() - $(this).outerHeight()) / 2, 'left': ($(window).width() - $(this).outerWidth() - 50), 'margin-top': '0px', 'margin-left': '0px'});

			if($(this).hasClass("middle")){
				$(this).css({'top': ($(window).height() - $(this).outerHeight()) / 2, 'left': ($(window).width() - $(this).outerWidth()) / 2, 'margin-top': '0px', 'margin-left': '0px'});
			}
	...
	$('.interface > .itf-head').mousedown(function(){$(this).addClass('active')}).mouseup(function(){$(this).removeClass('active')})
	...
	$('.needResize').resizable({
			...
			start: function(event, ui) {
				uiResizeCorrection[this] = {x: $(this).outerWidth() - $(this).width(),y: $(this).outerHeight() - $(this).height()};
			},
